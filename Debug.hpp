...
public:
    ...
    double similiarity(DistPrameter o);
    double similiarityNormal(DistPrameter o);
    double getVariance();
};
...
struct Node_UN{
    int vid;
    double  uniq;
    Node_UN(int v_id, float v_uniq):vid(v_id),uniq(v_uniq){};
};
...
//            size_t res=17;
//            res=res*31+ std::hash<int>()(m.first);
//            res=res*31+ std::hash<int>()(m.second);
            size_t res=std::hash<int>()(m.first)^(std::hash<int>()(m.second)<<1);
            return res;
        }
    };
}
...
void readDistParameter(string file, vector<DistPrameter> &vs);  //
